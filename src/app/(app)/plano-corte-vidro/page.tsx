...
import { PlanoCorteVidroForm } from "@/components/plano-corte/plano-corte-vidro-form";
import { formatDateTime } from "@/lib/utils";

export default async function PlanoCorteVidroPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-heading text-2xl font-bold text-primary">Plano de Corte de Vidro</h1>
        <p className="text-sm text-muted">
          {planos?.length ?? 0} plano(s) · empacotamento das peças dentro de chapas pra minimizar
          desperdício (algoritmo de prateleiras).
        </p>
      </div>
        ...
        <table className="w-full min-w-[640px] text-sm">
          <thead className="border-b border-border bg-background text-left text-xs uppercase tracking-wide text-muted">
            <tr>
              ...
              <th className="px-4 py-3">Chapas</th>
          ...
          <tbody>
            {(planos ?? []).map((p) => {
              const resultado = p.resultado as { totalChapas?: number; aproveitamentoMedioPercentual?: number };
              return (
                <tr key={p.id} className="border-b border-border last:border-0 hover:bg-background">
                  <td className="px-4 py-3">
                    <Link href={`/plano-corte-vidro/${p.id}`} className="font-medium text-primary hover:text-accent-dark">
                      {p.nome || "Plano de corte"}
                    </Link>
                  </td>
                  <td className="px-4 py-3 text-muted">{p.obras?.nome ?? "—"}</td>
                  <td className="px-4 py-3 text-muted">{resultado?.totalChapas ?? "—"}</td>
      ...
      <div className="rounded-2xl border border-border bg-surface p-6">
        ...
        <PlanoCorteVidroForm obras={obras ?? []} />
      </div>
    </div>
  );
}
