...
export default async function CalculosSacadaPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-heading text-2xl font-bold text-primary">Calculadora de Sacada</h1>
        ...
        <table className="w-full min-w-[620px] text-sm">
          <thead className="border-b border-border bg-background text-left text-xs uppercase tracking-wide text-muted">
            <tr>
              ...
              <th className="px-4 py-3">Painéis</th>
          ...
          <tbody>
            {(calculos ?? []).map((c) => {
              const resultado = c.resultado as { pesoTotalKg?: number; totalVidros?: number };
              const paineis = c.quantidade_paineis ?? resultado?.totalVidros ?? "—";
              return (
                <tr key={c.id} className="border-b border-border last:border-0 hover:bg-background">
                  <td className="px-4 py-3">
                    <Link href={`/calculadoras/sacada/${c.id}`} className="font-medium text-primary hover:text-accent-dark">
                      {c.nome || "Sem identificação"}
                    </Link>
                  </td>
                  <td className="px-4 py-3 text-muted uppercase">{c.tipo_sacada}</td>
                  <td className="px-4 py-3 text-muted">{c.kits?.nome ?? "—"}</td>
                  <td className="px-4 py-3 text-muted">{c.obras?.nome ?? "—"}</td>
                  <td className="px-4 py-3 text-muted">{paineis}</td>
            ...
            {(!calculos || calculos.length === 0) && (
              <tr>
                <td colSpan={7} className="px-4 py-10 text-center text-muted">
                  Nenhum cálculo de sacada ainda.
                </td>
              </tr>
            )}
          </tbody>
