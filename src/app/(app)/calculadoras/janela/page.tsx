...
const TIPO_LABEL: Record<string, string> = { fixa: "Fixa", correr: "De correr" };

export default async function CalculosJanelaPage() {
              ...
              return (
                <tr key={c.id} className="border-b border-border last:border-0 hover:bg-background">
                  <td className="px-4 py-3">
                    <Link href={`/calculadoras/janela/${c.id}`} className="font-medium text-primary hover:text-accent-dark">
                      {c.nome || "Sem identificação"}
                    </Link>
                  </td>
                  <td className="px-4 py-3 text-muted">{TIPO_LABEL[c.tipo_janela] ?? c.tipo_janela}</td>
            ...
            {(!calculos || calculos.length === 0) && (
              <tr>
                <td colSpan={6} className="px-4 py-10 text-center text-muted">
                  Nenhum cálculo de janela ainda.
                </td>
              </tr>
            )}
          </tbody>
