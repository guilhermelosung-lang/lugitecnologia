...
import type { ResultadoEspelhoTampo } from "@/lib/calc/espelho-tampo";
import { formatDateTime } from "@/lib/utils";

const TIPO_LABEL: Record<string, string> = { espelho: "Espelho", tampo: "Tampo", prateleira: "Prateleira" };
const ACABAMENTO_LABEL: Record<string, string> = {
  lapidada: "Lapidada",
  polida: "Polida",
  boleada: "Boleada",
...
export default async function EspelhoTampoDetalhePage({
  params,
}: {
  ...
  const resultado = calculo.resultado as unknown as ResultadoEspelhoTampo;

  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <p className="text-sm text-muted">
          {TIPO_LABEL[calculo.tipo]} · {calculo.obras?.nome ?? "Sem obra"} · Borda {ACABAMENTO_LABEL[calculo.acabamento_borda]}
        </p>
        <h1 className="font-heading text-2xl font-bold text-primary">{calculo.nome || "Cálculo"}</h1>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <div className="rounded-2xl border border-border bg-surface p-4">
          <p className="text-xs uppercase tracking-wide text-muted">Perímetro (borda)</p>
          <p className="font-heading text-lg font-bold text-primary">{resultado.perimetroMm} mm</p>
        </div>
        <div className="rounded-2xl border border-border bg-surface p-4">
      ...
      {calculo.quantidade_furos_fixacao > 0 && (
        <div className="rounded-2xl border border-border bg-surface p-4 text-sm text-foreground">
          {calculo.quantidade_furos_fixacao} furo(s) de fixação — posição definida na instalação conforme o suporte usado.
        </div>
      )}
