...
import { calcularESalvarSacadaUAction } from "@/lib/actions/calculo-sacada";
    ...
    <form action={action} className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="md:col-span-2">
          ...
          <Input id="nome" name="nome" placeholder="ex: Sacada U cobertura fundos" className="mt-1.5" />
        </div>
      </div>

      <div>
        <h3 className="mb-3 text-xs font-semibold uppercase tracking-wide text-muted">
          Segmentos (cada um calculado separadamente — nunca some os três vãos)
        </h3>
        <div className="space-y-4">
          <SegmentoCampos prefixo="esquerdo" rotulo="Lado Esquerdo (ponta na parede → encontro com a Frente)" pontaInicial="parede" pontaFinal="encontro" />
          <SegmentoCampos prefixo="frente" rotulo="Frente (encontro com Esquerdo → encontro com Direito)" pontaInicial="encontro" pontaFinal="encontro" />
          <SegmentoCampos prefixo="direito" rotulo="Lado Direito (encontro com a Frente → ponta na parede)" pontaInicial="encontro" pontaFinal="parede" />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <div>
            ...
            <Select id="abertura_segmento" name="abertura_segmento" defaultValue="frente" className="mt-1.5">
      ...
      <p className="text-xs text-muted">
        Cálculo assume os dois encontros de canto em 90°. Apenas vidro de segurança (temperado) é aceito
        para sacadas. Não substitui projeto estrutural ou responsabilidade técnica.
      </p>

      {state?.error && <p className="text-sm text-danger" role="alert">{state.error}</p>}

      <Button type="submit" disabled={pending}>
        {pending ? "Calculando..." : "Calcular cortina em U"}
      </Button>
    </form>
  );
}
