...
export function PortaResultado({
  resultado,
  ladoDobradica,
}: {
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-border bg-surface p-6">
        ...
        <PortaDesenho resultado={resultado} ladoDobradica={ladoDobradica} />
      </div>
      ...
      <div className="rounded-2xl border border-border bg-surface p-6">
          ...
          <tbody>
            {resultado.paineis.map((p) => (
              <tr key={p.identificacao} className="border-b border-border last:border-0">
                ...
                <td className="py-2 text-muted">{p.descricao}</td>
        ...
        <p className="mt-3 text-sm text-foreground">
          Peso total: <strong>{resultado.pesoTotalKg} kg</strong> · Abertura útil:{" "}
          <strong>{resultado.aberturaUtilMm} mm</strong>
        </p>
      </div>

      <div className="grid gap-6 md:grid-cols-2">
        <div className="rounded-2xl border border-border bg-surface p-6">
          <h2 className="mb-4 font-heading text-sm font-semibold text-primary">Furos e recortes</h2>
          <ul className="space-y-2 text-sm">
            {resultado.furos.map((f, i) => (
              <li key={i} className="flex justify-between">
                <span className="text-foreground">{f.descricao}</span>
                <span className="text-muted">
                  {f.distanciaBordaLateralMm} mm da borda · {f.distanciaBordaSuperiorMm} mm do topo
                </span>
              </li>
            ))}
          </ul>
        ...
        <div className="rounded-2xl border border-border bg-surface p-6">
          <h2 className="mb-4 font-heading text-sm font-semibold text-primary">Ferragens</h2>
          <ul className="space-y-2 text-sm">
            {resultado.ferragens.map((f, i) => (
              <li key={i} className="flex justify-between">
                <span className="text-foreground">{f.descricao}</span>
                <span className="text-muted">{f.quantidade}x</span>
              </li>
            ))}
          </ul>
        </div>
